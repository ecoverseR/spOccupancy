svcTPGOcc <- function(occ.formula, det.formula, data, inits, priors, 
  ...
  if (!NNGP) {
    stop("error: svcTPGOcc is currently only implemented for NNGP models. Please set NNGP = TRUE") 
  } else {
	...
	if (ar1) {
      ...
      if (p.svc == 1) {
        ...
        out$w.samples <- array(NA, dim = c(n.post.samples * n.chains, p.svc, J.w))
        out$w.samples[, 1, ] <- tmp
      } else {
	...
	storage.mode(X.w.fit) <- "double"
        consts.fit <- c(J.fit, n.obs.fit, p.occ, p.occ.re, n.occ.re.fit, 
			p.det, p.det.re, n.det.re.fit, n.years.max, p.svc, J.w.fit, ar1)
	...
	storage.mode(grid.fit.c) <- 'integer'
        ...
        out.fit <- .Call("svcTPGOccNNGP", y.fit, X.fit, X.w.fit, X.p.fit, coords.fit, 
			 X.re.fit, X.p.re.fit, 
			 ...
			 phi.inits, sigma.sq.inits, nu.inits,
                         w.inits.fit, z.inits.fit, z.long.indx.fit, z.year.indx.fit,
                         z.dat.indx.fit,  
	...
	dimnames(X.0)[[3]] <- tmp.names
        out.pred <- predict.svcTPGOcc(out.fit, X.0, coords.0, t.cols = 1:n.years.max, 
				      verbose = FALSE, grid.indx.0 = grid.0)
